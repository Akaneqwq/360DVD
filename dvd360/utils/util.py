...
from dvd360.utils.convert_from_ckpt import (
    convert_ldm_unet_checkpoint,
    convert_ldm_clip_checkpoint,
    convert_ldm_vae_checkpoint,
)
from dvd360.utils.convert_lora_safetensor_to_diffusers import (
    convert_lora,
...
def load_weights(
    ...
    if motion_module_path != "":
        ...
        # load from whole checkpoint
        # unet_state_dict.update({name[7:]: param for name, param in motion_module_state_dict.items() if "motion_modules." in name})
    ...
    # 1.1.1 motion adapter
    if motion_adapter != "":
        print(f"load motion adapter from {motion_adapter}")
        motion_adapter_state_dict = torch.load(motion_adapter, map_location="cpu")
        motion_adapter_state_dict = (
            motion_adapter_state_dict["state_dict"]
            if "state_dict" in motion_adapter_state_dict
            else motion_adapter_state_dict
        )
        motion_adapter_state_dict = {
            name[7:]: param
            for name, param in motion_adapter_state_dict.items()
            if "module." in name
        }
        animation_pipeline.motion_adapter.load_state_dict(motion_adapter_state_dict, strict=False)
        del motion_adapter_state_dict
